...
# Install the source package in the existing NeMo environment (repository root).
uv pip install --python .local-data/asr-nemo-venv/bin/python --no-deps -e child_speech_recognition

.local-data/asr-nemo-venv/bin/python -m child_speech.cli.train
# Run only after Validation selection is final:
.local-data/asr-nemo-venv/bin/python -m child_speech.cli.evaluate_test
```
...
```sh
.local-data/asr-nemo-venv/bin/python -m unittest discover -s child_speech_recognition/tests
```
